...
plot_brier_curves <- function(b, title, ylim_up = 1) {
  ggplot() + 
    ...
    ylim(c(0,ylim_up)) +
    xlab("Exposure time") + ylab("Prediction error") + ggtitle(title) 
}
...
plot_all_briers <- function(b0,b1,b2,b3,b4,b5,b6, stat = "median", ylim_up = 0.4, splitMethod, times) {
  ...
  ggplot() +
    ...
    scale_color_manual(values = c("BeSS" = "#1B9E77", "Lasso" = "#D95F02", "Elastic net" = "#7570B3", 
                                  ...
                                  "Reference" = "black"),
                       name = "Frailty models with\nvariables selected by") +
    xlim(0, times[[length(times)]]) + ylim(0, ylim_up) +
    xlab("Exposure time") + ylab("Prediction error") + 
...
plot_Brier_Curves <- function(pec_bess, pec_lasso, pec_enet, pec_ridge, pec_grouplasso, pec_coxboost, splitMethod) {
  ...
  # p5 <- plot_brier_curves(b5, paste0(splitMethod, " Brier Score of\nRpart method"), ylim_up = ylim_up)
  
  plot_all_briers(b0=NULL,b1,b2,b3,b4,b5,b6, stat="mean", 0.28, splitMethod = splitMethod, times = times)
}
...
ggsave(plot = p3, filename = "Codes/Simulations/Results/meanBrierCurves_verysmallsize.pdf", width = 35, height = 12)
...
p3 <- grid.arrange(p1, p2, ncol = 2)
ggsave(plot = p3, filename = "Codes/Simulations/Results/meanBrierCurves_smallsize.pdf", width = 35, height = 12)
...
p3 <- grid.arrange(p1, p2, ncol = 2)
ggsave(plot = p3, filename = "Codes/Simulations/Results/meanBrierCurves_largesize.pdf", width = 35, height = 12)
...
p3 <- grid.arrange(p1, p2, ncol = 2)
ggsave(plot = p3, filename = "Codes/Simulations/Results/meanBrierCurves_verylargesize.pdf", width = 35, height = 12)
