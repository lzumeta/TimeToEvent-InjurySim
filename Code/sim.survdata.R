...
sim.survdata <- function(N=1000, T=100, type="none", hazard.fun = NULL, num.data.frames = 1,
                         ...
                         X=NULL, beta=NULL, frailty = NULL, cluster.num = NULL, cluster.sizes=NULL, w=NULL, xvars=3, mu=0, sd=.5, ## frailty = "normal", "gamma"; K=int, numeric vector or, "uniform" or "poisson"
  ...
  result <- lapply(1:num.data.frames, FUN=function(i){  # for each replication
    ...
    me <- make.margeffect(baseline, xb, covariate, low, high, compare=compare, frailty=frailty)
    ...
    ## Censoring ------------- CHANGE
    if (type=="none" | type=="tvbeta") {
      if (!is.null(frailty)) {  ## if frailty
        
        data$failed[data$y == T] <- FALSE
        beforehand_censored <- mean(data$y == T) 
        new_censor <- censor - beforehand_censored
        new_censor <- (new_censor*N)/(N-sum(data$y == T))
        
        ifelse(censor.cond,    ## FALSE 
               data$failed <- !censor.x(xdata, censor=censor),
               data$failed[data$y != T] <- !(runif(sum(data$y != T)) < new_censor))  ## 
      } else {
        ifelse(censor.cond,
               data$failed <- !censor.x(xdata, censor=censor),
               data$failed <- !(runif(N) < censor))
