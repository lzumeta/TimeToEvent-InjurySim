...
#' me$marg.effect
make.margeffect <- function(baseline, xb, covariate=1, low=0, high=1, compare=median, frailty=frailty){
  
  if (!is.null(frailty)) xb$data <- dplyr::select(xb$data, -rep, -family)
