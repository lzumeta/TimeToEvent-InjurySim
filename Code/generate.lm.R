...
generate.lm <- function(baseline, X=NULL, beta = NULL, N=1000, type="none", xvars=3, mu=0, sd=1, censor=.1, frailty=NULL, cluster.num=NULL, cluster.sizes=NULL, w=NULL){ ## CHANGE
  T <- max(baseline$time)
  if(type=="none"){
    ...
    # ## Generate the frailty term for each cluster family (id.jug)  ## CHANGE -------
    if (!is.null(frailty)) {
      if (is.null(w)) { ## if user does not specify the frailty
        if (frailty == "gamma") {
          cluster.frailty <- rgamma(cluster.num, shape=0.8, scale=0.4)  
          #cluster.frailty <- rgamma(cluster.num, shape=1, scale=0.01)
        } else if (frailty == "normal") {
          cluster.frailty <- exp(rnorm(cluster.num, sd = 0.3))
        } else {
          stop("frailty must be one of gamma or normal")
        }
    ...
    if (!is.null(frailty)) {
      data$family <- rep(1:cluster.num, cluster.sizes)
      data$rep <- unlist(lapply(cluster.sizes, function(m) rep(1:m)))
    }
    
    tvc <- FALSE
    
  } else if(type=="tvc"){   ## Case: time-varying covariates
  ...
  } else if(type=="tvbeta"){ ## Case: time-varying betas
  ...
  return(list(data=data, beta=beta, XB=XB, exp.XB = exp(XB), survmat=survival, tvc=tvc, w=w))
}